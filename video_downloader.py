...
# Globals
video_segment_list, audio_segment_list = [], []
Reso_list, Aud_list = [], []
vid_resolution, aud_resolution = [], []
    ...
    if st.session_state.get("checked", False):
        try:
            ...
            if download:
                ...
                # Create throttles
                video_throttles = create_throttles(video_resolution[2])
                audio_throttles = create_throttles(audio_resolution[2])

                # Create range lists
                range_list(video_resolution[2], video_throttles, "Video")
                range_list(audio_resolution[2], audio_throttles, "Audio")

                # Download
                with ThreadPoolExecutor() as executor:
                    v_future = executor.submit(download_segments, video_throttles, video_stream,
                                               video_segment_list, video_file, total_size)
                    a_future = executor.submit(download_segments, audio_throttles, audio_stream,
                                               audio_segment_list, audio_file, total_size)

                    while not (v_future.done() and a_future.done()):
                        progress_bar.progress(min(int(percentage), 100))
                        downloading_status.text(f"Processed: {percentage}%")
                        time.sleep(0.5)

                # Debug check
                st.write("Video exists:", os.path.exists(video_file))
                st.write("Audio exists:", os.path.exists(audio_file))
                ...
                if os.path.exists(video_file) and os.path.exists(audio_file):
                    ...
                    st.error("Download failed! Segments did not merge into files.")
